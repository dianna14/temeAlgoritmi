...
def read_data_from_file(filename):
    ...
    # Citirea matricei Fjk
    matrix_data_Fjk = ''
    for line in lines:
        if 'Fjk' in line:  # Căutăm linia care conține Fjk
            matrix_data_Fjk = line.split('=')[1].strip().strip(';')  # Extragem partea cu valorile
            break

    # Acum combinăm toate liniile care conțin matricea Fjk
    for line in lines[lines.index(line) + 1:]:
        if line.strip() == '':  # Ignorăm liniile goale
            continue
        if '];' in line:  # Dacă linia conține finalul matricei, oprim
            matrix_data_Fjk += line.split('];')[0].strip()
            break
        matrix_data_Fjk += line.strip()  # Adăugăm liniile continuate ale matricei

    # Înlocuim spațiile cu virgule și corectăm parantezele
    matrix_data_Fjk = matrix_data_Fjk.replace(' ', ',')  # Înlocuim spațiile cu virgule
    matrix_data_Fjk = matrix_data_Fjk.replace('][', '],[')  # Corectăm separarea rândurilor
    matrix_data_Fjk = f'{matrix_data_Fjk}]'  # Închid parantezele pentru a forma lista corectă

    try:
        Fjk = np.array(eval(matrix_data_Fjk))  # Evaluăm pentru a construi matricea Fjk
    except SyntaxError as e:
        print(f"Eroare de sintaxă la evaluarea matricei Fjk: {e}")
        Fjk = []

    return d, r, SCj, Fj, Dk, np.array(Cjk, dtype=float), np.array(Fjk, dtype=float)
...
def metoda_vogel(SCj, Dk, Cjk, Fj, Fjk):
    total_cost = 0
    ...
    total_cost += np.sum(Fjk * (Dk == 0))  # Adaugă costurile fixe pentru locațiile de desfacere
...
def rezolva_problema_transport(nume_fisier):
    ...
    d, r, SCj, Fj, Dk, Cjk, Fjk = read_data_from_file(nume_fisier)

    # Apelăm metoda Vogel
    start_time = time.time()
    allocations, total_cost, iteration = metoda_vogel(SCj, Dk, Cjk, Fj, Fjk)
    ...
    fisier_nou = 'rezultate_transport_fcd_fcr.xlsx'
...
# Testează funcția
rezolva_problema_transport("Lab01_FCD_FCR_medium_25.dat")
